...
local cache_tooltip = CreateFrame("GameTooltip", "HNEliteBattlePetsTooltip")
...
local function mob_name(id)
    if not name_cache[id] then
        ...
        if cache_tooltip:IsShown() then
            name_cache[id] = HNEliteBattlePetsTooltipTextLeft1:GetText()
        end
    end
...
local function work_out_texture(point)
    ...
    if not default_texture then
        local texture, _, _, left, right, top, bottom = GetAtlasInfo("WildBattlePet")
        default_texture = {
            icon = texture,
            tCoordLeft = left,
            tCoordRight = right,
